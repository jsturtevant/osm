...
func getUpstreamServiceCluster(downstreamIdentity identity.ServiceIdentity, config trafficpolicy.MeshClusterConfig, sidecarSpec configv1alpha2.SidecarSpec, td string, upstreamIdentities []identity.ServiceIdentity) *xds_cluster.Cluster {
	httpProtocolOptions := getHTTPProtocolOptions("")

	marshalledUpstreamTLSContext, err := anypb.New(
		envoy.GetUpstreamTLSContext(downstreamIdentity, config.Service, sidecarSpec, td, upstreamIdentities))
	if err != nil {
...
func upstreamClustersFromClusterConfigs(downstreamIdentity identity.ServiceIdentity, configs []*trafficpolicy.MeshClusterConfig, sidecarSpec configv1alpha2.SidecarSpec, td string, upstreamIdentities []identity.ServiceIdentity) []*xds_cluster.Cluster {
	var clusters []*xds_cluster.Cluster

	for _, c := range configs {
		clusters = append(clusters, getUpstreamServiceCluster(downstreamIdentity, *c, sidecarSpec, td, upstreamIdentities))
	}
	return clusters
}
